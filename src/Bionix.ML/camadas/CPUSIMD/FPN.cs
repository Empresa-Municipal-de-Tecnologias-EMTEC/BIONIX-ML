...
{
    public class FPN : Interfaces.IFPN
    {
        ...
        private Tensor Upsample2x(Tensor src, ComputacaoContexto ctx)
        {
            ...
            // SIMD fast path when CPUSIMD tensors are available
            var simdCtx = ctx as ComputacaoCPUSIMDContexto;
            var srcT = src as TensorCPUSIMD;
            var dstT = dst as TensorCPUSIMD;
            if (simdCtx != null && srcT != null && dstT != null)
            {
                var srcData = srcT.Data;
                var dstData = dstT.Data;
                ...
                for (int x = 0; x < w; x++)
                {
                    ...
                    int offA = (yy * (w * 2) + xx) * c;
                    int offB = (yy * (w * 2) + (xx + 1)) * c;
                    int offC = ((yy + 1) * (w * 2) + xx) * c;
                    int offD = offC + c;
                    int ch = 0;
                    for (; ch <= c - vecSize; ch += vecSize)
                    {
                        var v = new System.Numerics.Vector<double>(srcData, baseSrc + ch);
                        v.CopyTo(dstData, offA + ch);
                        v.CopyTo(dstData, offB + ch);
                        v.CopyTo(dstData, offC + ch);
                        v.CopyTo(dstData, offD + ch);
                    }
                    for (; ch < c; ch++)
                    {
                        double v = srcData[baseSrc + ch];
                        dstData[offA + ch] = v;
                        dstData[offB + ch] = v;
                        dstData[offC + ch] = v;
                        dstData[offD + ch] = v;
                ...
                dstT.RequiresGrad = true;
                dstT.GradFn = new Bionix.ML.grafo.CPUSIMD.UpsampleFunction(src, dst);
                return dst;
            }
            ...
            dst.GradFn = new Bionix.ML.grafo.CPUSIMD.UpsampleFunction(src, dst);
            return dst;
        }
        ...
        private Tensor AddTensors(Tensor a, Tensor b, ComputacaoContexto ctx)
        {
            ...
            outSame.GradFn = new Bionix.ML.grafo.CPUSIMD.AddFunction(a, b, outSame);
            return outSame;
        }
    }
}
