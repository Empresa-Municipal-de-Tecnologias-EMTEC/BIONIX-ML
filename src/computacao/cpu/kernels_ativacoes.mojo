...
fn identidade_cpu(x: tensor_defs.Tensor) -> tensor_defs.Tensor:
    return x.copy()


fn derivada_identidade_cpu(entrada: tensor_defs.Tensor, grad_saida: tensor_defs.Tensor) -> tensor_defs.Tensor:
...
fn relu_cpu(x: tensor_defs.Tensor) -> tensor_defs.Tensor:
...
fn derivada_relu_cpu(entrada: tensor_defs.Tensor, grad_saida: tensor_defs.Tensor) -> tensor_defs.Tensor:
    debug_assert(len(entrada.dados) == len(grad_saida.dados), "gradiente incompatível")
    var formato = entrada.formato.copy()
    var grad = tensor_defs.Tensor(formato^, entrada.tipo_computacao)
...
fn hard_sigmoid_cpu(x: tensor_defs.Tensor) -> tensor_defs.Tensor:
    var formato = x.formato.copy()
    var out = tensor_defs.Tensor(formato^, x.tipo_computacao)
    for i in range(len(x.dados)):
...
fn derivada_hard_sigmoid_cpu(entrada: tensor_defs.Tensor, grad_saida: tensor_defs.Tensor) -> tensor_defs.Tensor:
    debug_assert(len(entrada.dados) == len(grad_saida.dados), "gradiente incompatível")
    var formato = entrada.formato.copy()
    var grad = tensor_defs.Tensor(formato^, entrada.tipo_computacao)
    for i in range(len(entrada.dados)):
