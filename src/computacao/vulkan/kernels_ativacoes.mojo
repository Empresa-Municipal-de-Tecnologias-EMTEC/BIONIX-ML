...
fn identidade_vulkan(x: tensor_defs.Tensor) -> tensor_defs.Tensor:
    return x.copy()


fn derivada_identidade_vulkan(entrada: tensor_defs.Tensor, grad_saida: tensor_defs.Tensor) -> tensor_defs.Tensor:
...
fn relu_vulkan(x: tensor_defs.Tensor) -> tensor_defs.Tensor:
...
fn derivada_relu_vulkan(entrada: tensor_defs.Tensor, grad_saida: tensor_defs.Tensor) -> tensor_defs.Tensor:
    debug_assert(len(entrada.dados) == len(grad_saida.dados), "gradiente incompatível")
    var formato = entrada.formato.copy()
    var grad = tensor_defs.Tensor(formato^, entrada.tipo_computacao)
...
fn hard_sigmoid_vulkan(x: tensor_defs.Tensor) -> tensor_defs.Tensor:
    var formato = x.formato.copy()
    var out = tensor_defs.Tensor(formato^, x.tipo_computacao)
    for i in range(len(x.dados)):
...
fn derivada_hard_sigmoid_vulkan(entrada: tensor_defs.Tensor, grad_saida: tensor_defs.Tensor) -> tensor_defs.Tensor:
    debug_assert(len(entrada.dados) == len(grad_saida.dados), "gradiente incompatível")
    var formato = entrada.formato.copy()
    var grad = tensor_defs.Tensor(formato^, entrada.tipo_computacao)
    for i in range(len(entrada.dados)):
